...
path = Path("/home/victor/USP/TCC_IC/akara_2024/resultados/v1.0.0/default/2024021400/mslp/")
path2 = Path("/home/victor/USP/TCC_IC/akara_2024/resultados/v1.1.0/default/2024021400/mslp/")
...
limit_date = "20240217_060000"

df_filtrado = df_all[df_all["FCST_VALID"] > limit_date]
df_filtrado = df_filtrado.reset_index(drop=True)

linhas_excluidas = [1,2,  20, 21, 24]
# Exclui linhas com índices 1, 2 e 5
df_filtrado = df_filtrado.drop(index=linhas_excluidas).reset_index(drop=True)

print(df_filtrado[["FCST_VALID", "OBJECT_ID","LENGTH",
              "WIDTH", "AREA", "CENTROID_X", "CENTROID_Y",
                "CENTROID_LAT", "CENTROID_LON", "AXIS_ANG", 
                "CURVATURE", "COMPLEXITY"]])
...
linhas_excluidas = [5, 26]
# Exclui linhas com índices 1, 2 e 5
df_filtrado2 = df_filtrado2.drop(index=linhas_excluidas).reset_index(drop=True)
print(df_filtrado2[["FCST_VALID", "OBJECT_ID","LENGTH",
              "WIDTH", "AREA", "CENTROID_X", "CENTROID_Y",
                "CENTROID_LAT", "CENTROID_LON", "AXIS_ANG", 
                "CURVATURE", "COMPLEXITY"]])


outdir = Path("/home/victor/USP/TCC_IC/akara_2024/resultados/comparative/2024021400/mslp")
...
df_filtrado_F = df_filtrado[df_filtrado["OBJECT_ID"].str.startswith("F")]

# Filtrar linhas cujo OBJECT_ID começa com "O"
df_filtrado_O = df_filtrado[df_filtrado["OBJECT_ID"].str.startswith("O")]

# Mesma lógica para o segundo dataframe
df_filtrado2_F = df_filtrado2[df_filtrado2["OBJECT_ID"].str.startswith("F")]
df_filtrado2_O = df_filtrado2[df_filtrado2["OBJECT_ID"].str.startswith("O")]

grade = 55
grade1 = 55*55
# ---------- Área ----------
plt.figure(figsize=(10,6))
plt.plot(df_filtrado_F["datetime"], df_filtrado_F["AREA"]*grade1, "-o", label="Forecast v1.0.0", color='#65a1e6')
plt.plot(df_filtrado2_F["datetime"], df_filtrado2_F["AREA"]*grade1, "x", label="Forecast v1.1.0", color='#65a1e6', linestyle='--')
...
grade = 55
plt.figure(figsize=(10,6))
plt.plot(df_filtrado_F["datetime"], df_filtrado_F["LENGTH"]*grade, "-o", label="Forecast v1.0.0", color='#65a1e6')
plt.plot(df_filtrado2_F["datetime"], df_filtrado2_F["LENGTH"]*grade, "x", label="Forecast v1.1.0", color='#65a1e6', linestyle='--')
...
# ---------- Width ----------
plt.figure(figsize=(10,6))
plt.plot(df_filtrado_F["datetime"], df_filtrado_F["WIDTH"]*grade, "-o", label="Forecast v1.0.0", color='#65a1e6')
plt.plot(df_filtrado2_F["datetime"], df_filtrado2_F["WIDTH"]*grade, "x", label="Forecast v1.1.0", color='#65a1e6', linestyle='--')
